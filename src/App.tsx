...
const DEFAULT_GAME: Game = {
  title: '新港八卦謎蹤 - 創作版',
  stages: [{
    id: 's1', order: 1, title: '時空的起點', speaker: '白鸞卿', storyContent: '請在後台拍照並上傳，為我設定專屬的時空背景。', imageUrl: '', unlockType: 'PASSWORD', unlockAnswer: '1234', hints: ['初始密碼 1234'], successMessage: '設定完成！'
  }]
};
...
export default function App() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("上傳失敗，請確認 Firebase Storage 權限。");
    } finally {
      setUploading(false);
    }
  };
  ...
  if (loading) return <div className="h-screen bg-slate-950 flex items-center justify-center text-amber-500 font-black animate-pulse uppercase tracking-[0.5em]">Syncing Universe...</div>;
  ...
  return (
    <div className="h-screen w-full bg-black overflow-hidden relative">
      ...
      <div className="absolute top-10 left-6 right-6 z-10 flex justify-between items-center pointer-events-none">
         <div className="bg-black/60 backdrop-blur-xl px-6 py-3 rounded-full border border-white/10 shadow-2xl pointer-events-auto text-white font-black text-sm tracking-widest">{game.title}</div>
                      ...
                      <div className="space-y-5">
                         {currentStage.unlockType === 'PASSWORD' ? (
                           <div className="relative group text-white">
                             <input type="text" value={userInput} onChange={e => setUserInput(e.target.value)} placeholder="解密..." className="w-full bg-black/40 border-2 border-white/5 rounded-[24px] py-5 px-8 outline-none focus:border-amber-500 shadow-inner" />
                             <button onClick={() => { if(userInput.trim().toLowerCase() === currentStage.unlockAnswer.toLowerCase()) setSolved(true); else alert('無效的代碼'); }} className="absolute right-2 top-2 bottom-2 bg-amber-500 text-black px-8 rounded-2xl font-black">OK</button>
                           </div>
      ...
      {isAdmin && (
        <div className="absolute inset-0 z-[1000] bg-slate-50 overflow-y-auto text-slate-900 pb-20">
           {!isLogged ? (
             <div className="h-screen bg-black flex items-center justify-center p-8 text-white text-center"><div className="bg-slate-900 p-12 rounded-[60px] shadow-2xl w-full max-w-sm border border-white/5 space-y-10"><h1 className="text-3xl font-black tracking-[0.3em]">ADMIN LOGIN</h1><input type="password" value={adminPass} onChange={e => setAdminPass(e.target.value)} placeholder="••••" className="w-full bg-black border-2 border-slate-800 rounded-3xl py-6 px-6 outline-none focus:border-amber-500 text-center font-mono text-3xl text-amber-500 tracking-[0.5em]" /><button onClick={() => { if(adminPass === '8888') setIsLogged(true); else alert('密碼錯誤'); }} className="w-full bg-amber-500 text-black py-5 rounded-[30px] font-black shadow-xl">ENTER CORE</button><button onClick={() => setIsAdmin(false)} className="text-slate-600 font-bold uppercase text-[10px] tracking-widest">Cancel</button></div></div>
           ) : (
             <div className="max-w-4xl mx-auto p-10">
                {editingStage && (
                  <div className="fixed inset-0 bg-black/95 z-[2000] flex items-center justify-center p-4 overflow-y-auto text-slate-900 text-center">
                    <div className="bg-white w-full max-w-xl rounded-[60px] p-12 space-y-8 shadow-2xl">
                       <div className="flex justify-between items-center"><h3 className="text-2xl font-black italic text-slate-900">MISSION EDITOR</h3><button onClick={() => setEditingStage(null)} className="p-3 hover:bg-slate-100 rounded-full"><X/></button></div>
                       <div className="space-y-6 text-left">
                          {/* 圖片上傳區 */}
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">任務場景照片 (手機拍照/上傳)</label>
                             <div 
                                onClick={() => fileInputRef.current?.click()}
                                className="h-48 w-full rounded-[32px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 transition-all overflow-hidden relative"
                             >
                                {editingStage.imageUrl ? (
                                  <img src={editingStage.imageUrl} className="w-full h-full object-cover" alt="Preview" />
                                ) : (
                                  <div className="text-center text-slate-300">
                                     {uploading ? <Loader2 className="animate-spin mx-auto mb-2" /> : <ImageIcon size={48} className="mx-auto mb-2"/>}
                                     <p className="font-black text-sm">點擊上傳或拍照</p>
                                  </div>
                                )}
                                {uploading && <div className="absolute inset-0 bg-white/80 flex items-center justify-center font-black text-indigo-600">上傳中...</div>}
                             </div>
                          ...
                          <div className="space-y-2 text-slate-900"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-slate-900">對話框內容</label><textarea className="w-full bg-slate-50 rounded-[32px] p-6 h-40 outline-none text-base leading-relaxed text-slate-900 shadow-inner" value={editingStage.storyContent} onChange={e => setEditingStage({...editingStage, storyContent: e.target.value})} /></div>
                          
                          <div className="grid grid-cols-2 gap-6">
                             <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase text-left text-slate-900">解鎖方式</label><select className="w-full border-b-2 py-3 bg-white text-slate-900 font-bold" value={editingStage.unlockType} onChange={e => setEditingStage({...editingStage, unlockType: e.target.value as any})}><option value="PASSWORD">密碼</option><option value="GPS">GPS</option><option value="QR_CODE">QR掃描</option></select></div>
                          ...
                          }} className="w-full bg-slate-900 text-white py-6 rounded-[32px] font-black text-xl active:scale-95 transition-all">更新時空檔案</button>
                       </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-between items-end mb-20 text-slate-900 text-left">
                   ...
                   <div className="flex gap-4">
                      <button onClick={async () => { await setDoc(doc(db, "games", "shinkang_v4"), game); alert("全球同步已完成！"); }} className="bg-green-600 text-white px-10 py-5 rounded-[30px] font-black flex items-center gap-3 shadow-2xl active:scale-95 transition-all italic tracking-tighter"><Save size={24}/> SYNC TO CLOUD</button>
                   ...
                   <div className="grid gap-6">
                      {game.stages.map((s, i) => (
                        <div key={s.id} className="bg-white p-10 rounded-[60px] border-2 border-transparent hover:border-indigo-100 flex items-center gap-10 transition-all group shadow-sm text-slate-900 text-left">
                           ...
                           <div className="flex-1 text-slate-900 text-left"><h4 className="text-2xl font-black text-slate-800 tracking-tight text-left">{s.title}</h4><div className="flex gap-3 mt-3 text-slate-400 text-left text-[10px] font-black uppercase tracking-widest"><span>{s.unlockType}</span><span>Key: {s.unlockAnswer}</span></div></div>
                           <div className="flex gap-4"><button onClick={() => setEditingStage(s)} className="p-6 bg-slate-50 text-slate-400 hover:bg-indigo-600 hover:text-white rounded-[30px] transition-all active:scale-90"><Edit2 size={28}/></button><button onClick={() => { if(confirm('確定要移除？')) setGame({...game, stages: game.stages.filter(item => item.id !== s.id)}) }} className="p-6 bg-slate-50 text-slate-400 hover:bg-red-500 hover:text-white rounded-[30px] transition-all active:scale-90"><Trash2 size={28}/></button></div>
                        </div>
                      ))}
                   </div>
                </div>
      ...
      {panelMode === 'closed' && !isAdmin && (
        <div className="absolute bottom-12 left-0 right-0 z-10 flex justify-center animate-bounce"><button onClick={() => setPanelMode('story')} className="bg-amber-500 text-black px-10 py-4 rounded-full font-black flex items-center gap-3 shadow-2xl shadow-amber-900/50">開啟任務面板 <ChevronUp size={24} /></button></div>
      )}
    </div>
  );
}
