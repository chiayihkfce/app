...
import { BookOpen, Map as MapIcon, Briefcase, Settings, ChevronRight, MapPin, CheckCircle2, Trash2, Edit2, ArrowLeft, X, QrCode, Save, RefreshCw, MessageCircle } from 'lucide-react';
        ...
        return (
          <div className="h-full relative animate-in fade-in duration-500">
                ...
                s.lat && s.lng && (
                  <Marker key={s.id} position={[s.lat, s.lng]} icon={createCustomIcon(idx === currentStageIdx ? '#f59e0b' : '#475569')}>
                    <Popup className="custom-popup">
                       <div className="p-3 text-center text-slate-900">
            ...
            <div className="absolute top-10 left-6 right-6 z-[1000] pointer-events-none">
               <div className="bg-black/60 backdrop-blur-xl p-5 rounded-[32px] border border-white/5 shadow-2xl inline-block text-white">
                  <h3 className="font-black text-xs uppercase tracking-[0.2em] mb-1">Real-time Radar</h3>
          ...
          <div className="h-full bg-black p-10 pt-20 animate-in slide-in-from-bottom duration-500">
             ...
             {inventory.length === 0 ? (
               <div className="mt-20 flex flex-col items-center justify-center opacity-20">
                  <div className="w-32 h-32 border-4 border-dashed border-slate-800 rounded-full flex items-center justify-center mb-6 text-white">
                     <Briefcase size={48} />
                  </div>
               ...
               <div className="grid grid-cols-2 gap-6">
                  {inventory.map((item, i) => (
                    <div key={i} className="bg-slate-900/50 border border-slate-800 p-8 rounded-[40px] flex flex-col items-center gap-4 shadow-2xl active:scale-95 transition-all text-white">
                       <div className="w-20 h-20 bg-amber-500/10 rounded-[24px] flex items-center justify-center text-amber-500">
                          <BookOpen size={32} />
                       </div>
                       <span className="font-black text-sm tracking-wide">{item}</span>
                    </div>
                  ))}
               </div>
             )}
      ...
      case 'admin':
        return (
          <div className="h-full bg-slate-950 flex flex-col items-center justify-center p-10 text-center text-white">
             ...
             <h2 className="text-2xl font-black mb-4">系統核心設定</h2>
    ...
    if (!isLogged) return (
      <div className="h-screen bg-black flex items-center justify-center p-8 text-white">
        <div className="bg-slate-900 p-12 rounded-[50px] shadow-2xl w-full max-w-sm text-center space-y-8 border border-white/5">
           <h1 className="text-3xl font-black tracking-widest">AUTHENTICATION</h1>
      ...
      <div className="min-h-screen bg-slate-50 p-8 font-sans text-slate-900 pb-32">
        {editingStage && (
          <div className="fixed inset-0 bg-black/95 z-[2000] flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-white w-full max-w-xl rounded-[50px] p-10 space-y-6 shadow-2xl">
               <div className="flex justify-between items-center text-slate-900"><h3 className="text-2xl font-black italic">STAGE EDITOR</h3><button onClick={() => setEditingStage(null)} className="p-2 hover:bg-slate-100 rounded-full"><X/></button></div>
               <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase">標題</label><input className="w-full border-b-2 py-2 outline-none font-bold text-lg text-slate-900" value={editingStage.title} onChange={e => setEditingStage({...editingStage, title: e.target.value})} /></div>
                    <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase">說話者</label><input className="w-full border-b-2 py-2 outline-none font-bold text-lg text-slate-900" value={editingStage.speaker} onChange={e => setEditingStage({...editingStage, speaker: e.target.value})} /></div>
                  </div>
                  <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase">劇情故事</label><textarea className="w-full bg-slate-50 rounded-2xl p-5 h-32 outline-none text-sm leading-relaxed text-slate-900" value={editingStage.storyContent} onChange={e => setEditingStage({...editingStage, storyContent: e.target.value})} /></div>
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-400 uppercase">大圖網址</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                     <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase">解鎖類型</label><select className="w-full border-b-2 py-2 bg-white text-slate-900" value={editingStage.unlockType} onChange={e => setEditingStage({...editingStage, unlockType: e.target.value as any})}><option value="PASSWORD">密碼鎖</option><option value="GPS">GPS定位</option><option value="QR_CODE">QR掃描</option></select></div>
                     <div className="space-y-1"><label className="text-[9px] font-black text-slate-400 uppercase">正確答案</label><input className="w-full border-b-2 py-2 outline-none font-mono text-slate-900" value={editingStage.unlockAnswer} onChange={e => setEditingStage({...editingStage, unlockAnswer: e.target.value})} /></div>
                  </div>
                  <button onClick={() => {
        ...
        <div className="max-w-3xl mx-auto">
           <div className="flex justify-between items-center mb-16">
              <div><h1 className="text-4xl font-black text-slate-900">遊戲引擎後台</h1><p className="text-slate-400 text-xs font-bold uppercase tracking-[0.3em] mt-1 italic underline decoration-amber-500 text-slate-900">Cloud Sync Active</p></div>
              <div className="flex gap-3">
                 ...
                 <button onClick={() => { setIsAdmin(false); setIsLogged(false); }} className="bg-white border px-8 py-4 rounded-3xl font-bold flex items-center gap-2 shadow-sm active:scale-95 transition-all text-slate-900"><ArrowLeft size={20}/> 返回遊戲</button>
              </div>
           </div>
           
           <div className="space-y-6">
              <div className="bg-white p-10 rounded-[50px] shadow-sm border border-slate-100 flex items-center justify-between text-slate-900">
              ...
              <div className="grid gap-4">
                 {game.stages.map((s, i) => (
                   <div key={s.id} className="bg-white p-8 rounded-[40px] border-2 border-transparent hover:border-indigo-100 flex items-center gap-8 transition-all group shadow-sm overflow-hidden relative text-slate-900">
                      <div className="w-16 h-16 bg-slate-50 rounded-3xl flex items-center justify-center font-black text-slate-300 text-2xl group-hover:text-indigo-600 transition-all">{i+1}</div>
                      ...
                      <div className="flex gap-3 relative z-10 text-slate-400">
                         <button onClick={() => setEditingStage(s)} className="p-5 bg-slate-50 hover:bg-indigo-600 hover:text-white rounded-3xl transition-all shadow-sm"><Edit2 size={24}/></button>
                         <button onClick={() => { if(confirm('刪除？')) setGame({...game, stages: game.stages.filter(item => item.id !== s.id)}) }} className="p-5 bg-slate-50 hover:bg-red-500 hover:text-white rounded-3xl transition-all shadow-sm"><Trash2 size={24}/></button>
                      </div>
                   </div>
                 ))}
              </div>
    ...
    <div className="flex flex-col h-screen bg-black text-white overflow-hidden selection:bg-amber-500 selection:text-black">
      ...
      {isScanning && (
        <div className="fixed inset-0 bg-black z-[2000] flex flex-col items-center justify-center p-8 animate-in zoom-in duration-300">
           <div className="w-full max-w-sm aspect-square bg-slate-900 rounded-[50px] overflow-hidden border-4 border-amber-500 shadow-2xl mb-10">
