    ...
    <div className="h-screen w-full bg-black overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-12 text-center">
               {panelMode === 'story' && (
                 <div className="space-y-6">
                    ...
                    <div className="text-slate-200 text-lg leading-relaxed font-medium italic border-l-2 border-amber-500/30 pl-4 mb-10 text-left">
                      ...
                      <div className="space-y-5">
                         {currentStage.unlockType === 'PASSWORD' ? (
                           <div className="relative group text-white">
                             ...
                             <button onClick={() => { if(userInput.trim().toLowerCase() === currentStage.unlockAnswer.toLowerCase()) setSolved(true); else alert('無效的代碼'); }} className="absolute right-2 top-2 bottom-2 bg-amber-500 text-black px-8 rounded-2xl font-black shadow-lg active:scale-95 transition-all text-center">OK</button>
                           </div>
               ...
               {panelMode === 'backpack' && (
                 <div className="flex flex-col text-white">
                    <h2 className="text-4xl font-black mb-2 flex items-center justify-center gap-4 text-center"><Briefcase className="text-amber-500" size={40}/> 拾獲物</h2>
                    ...
                    {inventory.length === 0 ? (
                      ...
                      <div className="grid grid-cols-2 gap-6">{inventory.map((item, i) => (<div key={i} className="bg-slate-800/50 border border-white/5 p-10 rounded-[48px] flex flex-col items-center justify-center gap-5 text-white text-sm font-black shadow-2xl transition-all active:scale-95 text-center"><div className="w-20 h-20 bg-amber-500/10 rounded-[28px] flex items-center justify-center text-amber-500 shadow-inner"><BookOpen size={36}/></div><span className="tracking-wide text-center">{item}</span></div>))}</div>
                    )}
                 </div>
               )}
               ...
               {panelMode === 'admin' && (
                 <div className="flex flex-col items-center justify-center py-10 text-center text-white">
                    ...
                    <h2 className="text-3xl font-black mb-4 tracking-tight uppercase tracking-[0.2em] text-center">系統管理核心</h2>
                    <p className="text-slate-500 text-sm mb-12 font-medium leading-relaxed max-w-xs mx-auto text-center">僅限管理員進入以修改時空檔案。</p>
                    <button onClick={() => setIsAdmin(true)} className="bg-white text-black px-14 py-5 rounded-[24px] font-black shadow-2xl active:scale-95 transition-all tracking-widest uppercase text-center">Verify Identity</button>
                 </div>
               )}
            </div>
          </motion.div>
           ...
           {!isLogged ? (
             <div className="h-screen bg-black flex items-center justify-center p-8 text-white text-center">
               <div className="bg-slate-900 p-12 rounded-[60px] shadow-2xl w-full max-w-sm border border-white/5 space-y-10">
                  ...
                  <button onClick={() => { if(adminPass === '8888') setIsLogged(true); else alert('密碼錯誤'); }} className="w-full bg-amber-500 text-black py-5 rounded-[30px] font-black shadow-xl active:scale-95 transition-all text-center">ENTER CORE</button>
                  <button onClick={() => setIsAdmin(false)} className="text-slate-600 font-bold uppercase text-[10px] tracking-widest text-center">Cancel</button>
               </div>
             </div>
           ) : (
             <div className="max-w-4xl mx-auto p-10">
                {editingStage && (
                  <div className="fixed inset-0 bg-black/95 z-[2000] flex items-center justify-center p-4 overflow-y-auto text-slate-900 text-center">
                    <div className="bg-white w-full max-w-xl rounded-[60px] p-12 space-y-8 shadow-2xl text-center">
                       <div className="flex justify-between items-center"><h3 className="text-2xl font-black italic tracking-tighter text-slate-900">MISSION EDITOR</h3><button onClick={() => setEditingStage(null)} className="p-3 hover:bg-slate-100 rounded-full transition-colors text-slate-900"><X/></button></div>
                       <div className="space-y-6 text-left">
                          ...
                          <div className="space-y-2 text-slate-900"><label className="text-[10px] font-black text-slate-400 uppercase">圖片網址</label><input className="w-full border-b-2 py-3 outline-none text-sm text-blue-600 font-mono text-left" placeholder="https://..." value={editingStage.imageUrl} onChange={e => setEditingStage({...editingStage, imageUrl: e.target.value})} /></div>
                          <div className="grid grid-cols-2 gap-6 text-slate-900">
                             <div className="space-y-2 text-slate-900"><label className="text-[10px] font-black text-slate-400 uppercase text-left">解鎖方式</label><select className="w-full border-b-2 py-3 bg-white text-slate-900 font-bold" value={editingStage.unlockType} onChange={e => setEditingStage({...editingStage, unlockType: e.target.value as any})}><option value="PASSWORD">密碼鎖</option><option value="GPS">GPS定位鎖</option><option value="QR_CODE">QR掃碼鎖</option></select></div>
                             <div className="space-y-2 text-slate-900"><label className="text-[10px] font-black text-slate-400 uppercase text-left">正確答案</label><input className="w-full border-b-2 py-3 outline-none font-mono text-xl text-slate-900 font-bold" value={editingStage.unlockAnswer} onChange={e => setEditingStage({...editingStage, unlockAnswer: e.target.value})} /></div>
                          </div>
                          {editingStage.unlockType === 'GPS' && (
                            <div className="h-56 rounded-[32px] overflow-hidden border-2 shadow-lg text-slate-900 font-black flex items-center justify-center bg-slate-100 italic text-center">請在雲端資料庫設定座標 (Lat: {editingStage.lat}, Lng: {editingStage.lng})</div>
                          )}
                          <button onClick={() => {
                ...
                <div className="flex justify-between items-end mb-20 text-slate-900 text-left">
                   ...
                   <div className="flex gap-4">
                      <button onClick={async () => { await setDoc(doc(db, "games", "shinkang_v4"), game); alert("同步完成！"); }} className="bg-green-600 text-white px-10 py-5 rounded-[30px] font-black flex items-center gap-3 shadow-2xl active:scale-95 transition-all italic tracking-tighter text-center"><Save size={24}/> SYNC TO CLOUD</button>
                      ...
                      {game.stages.map((s, i) => (
                        <div key={s.id} className="bg-white p-10 rounded-[60px] border-2 border-transparent hover:border-indigo-100 flex items-center gap-10 transition-all group shadow-sm text-slate-900 text-left">
                           ...
                           <div className="flex-1 text-left">
                              ...
                              <div className="flex gap-3 mt-3 text-left text-slate-400">
                                 <span className="bg-slate-100 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest text-left">{s.unlockType}</span>
                                 <span className="bg-slate-100 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest text-left">Key: {s.unlockAnswer}</span>
                              </div>
                           </div>
                           <div className="flex gap-4">
                              <button onClick={() => setEditingStage(s)} className="p-6 bg-slate-50 text-slate-400 hover:bg-indigo-600 hover:text-white rounded-[30px] transition-all shadow-sm active:scale-90 text-center"><Edit2 size={28}/></button>
                              <button onClick={() => { if(confirm('確定要移除此任務？')) setGame({...game, stages: game.stages.filter(item => item.id !== s.id)}) }} className="p-6 bg-slate-50 text-slate-400 hover:bg-red-500 hover:text-white rounded-[30px] transition-all shadow-sm active:scale-90 text-center"><Trash2 size={28}/></button>
                           </div>
                        </div>
                      ))}
                   </div>
      ...
      {isScanning && (
        <div className="absolute inset-0 bg-black z-[2000] flex flex-col items-center justify-center p-10 animate-in zoom-in duration-300">
           ...
           <button onClick={() => setIsScanning(false)} className="bg-white/10 hover:bg-white/20 text-white px-14 py-6 rounded-[30px] font-black uppercase tracking-[0.3em] transition-all text-center">Abort Scanner</button>
        </div>
      )}

      {panelMode === 'closed' && !isAdmin && (
        <div className="absolute bottom-12 left-0 right-0 z-10 flex justify-center animate-bounce">
           <button onClick={() => setPanelMode('story')} className="bg-amber-500 text-black px-10 py-4 rounded-full font-black flex items-center gap-3 shadow-2xl shadow-amber-900/50 text-center">
