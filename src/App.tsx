...
import { BookOpen, Map as MapIcon, Briefcase, Settings, ChevronRight, CheckCircle2, Trash2, Edit2, X, QrCode, Save, ChevronUp, User, Image as ImageIcon, Loader2, RefreshCw, MessageCircle } from 'lucide-react';
...
export default function App() {
  ...
  if (loading) return <div className="h-screen bg-slate-950 flex items-center justify-center text-amber-500 font-black animate-pulse uppercase tracking-[0.5em] flex-col gap-4"><RefreshCw className="animate-spin" /><span>Syncing...</span></div>;
    ...
    <div className="h-screen w-full bg-black overflow-hidden relative">
            ...
            <div className="flex-1 overflow-y-auto px-8 pb-12 text-center">
               {panelMode === 'story' && (
                 <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-center gap-3 mb-2 text-amber-500 font-black tracking-widest"><div className="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center text-black shadow-lg"><User size={16}/></div>{currentStage.speaker}</div>
                    ...
                    {!solved ? (
                      <div className="space-y-5">
                         {currentStage.unlockType === 'PASSWORD' ? (
                           <div className="relative group text-white"><input type="text" value={userInput} onChange={e => setUserInput(e.target.value)} placeholder="在此輸入代碼..." className="w-full bg-black/40 border-2 border-white/5 rounded-[24px] py-5 px-8 outline-none focus:border-amber-500" /><button onClick={() => { if(userInput.trim().toLowerCase() === currentStage.unlockAnswer.toLowerCase()) setSolved(true); else alert('無效'); }} className="absolute right-2 top-2 bottom-2 bg-amber-500 text-black px-8 rounded-2xl font-black active:scale-95 transition-all">OK</button></div>
      ...
      {isAdmin && (
        <div className="absolute inset-0 z-[1000] bg-slate-50 overflow-y-auto text-slate-900 pb-20 font-sans">
           {!isLogged ? (
             <div className="h-screen bg-black flex items-center justify-center p-8 text-white text-center"><div className="bg-slate-900 p-12 rounded-[60px] shadow-2xl w-full max-w-sm border border-white/5 space-y-10"><h1 className="text-3xl font-black tracking-[0.3em]">ADMIN</h1><input type="password" value={adminPass} onChange={e => setAdminPass(e.target.value)} placeholder="••••" className="w-full bg-black border-2 border-slate-800 rounded-3xl py-6 px-6 outline-none focus:border-amber-500 text-center font-mono text-3xl text-amber-500 tracking-[0.5em]" /><button onClick={() => { if(adminPass === '8888') setIsLogged(true); else alert('密碼錯誤'); }} className="w-full bg-amber-500 text-black py-5 rounded-[30px] font-black shadow-xl">AUTHENTICATE</button><button onClick={() => setIsAdmin(false)} className="text-slate-600 font-bold uppercase text-[10px] tracking-widest">Return</button></div></div>
           ) : (
             <div className="max-w-4xl mx-auto p-10">
                    ...
                    <div className="bg-white w-full max-w-xl rounded-[60px] p-12 space-y-8 shadow-2xl">
                       ...
                       <div className="space-y-6 text-left">
                          <div className="space-y-2">
                             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-slate-900">任務背景照片 (點擊上傳)</label>
                             <div onClick={() => fileInputRef.current?.click()} className="h-48 w-full rounded-[32px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center cursor-pointer hover:bg-slate-50 transition-all overflow-hidden relative">
                                ...
                                {uploading && <div className="absolute inset-0 bg-white/80 flex items-center justify-center font-black text-indigo-600 animate-pulse">上傳中...</div>}
                             </div>
                          ...
                          <div className="grid grid-cols-2 gap-6 text-slate-900">
                            ...
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">發話者 (角色名稱)</label><input className="w-full border-b-2 py-3 outline-none font-bold text-xl text-slate-900" value={editingStage.speaker} onChange={e => setEditingStage({...editingStage, speaker: e.target.value})} /></div>
                          </div>
                          ...
                          }} className="w-full bg-slate-900 text-white py-6 rounded-[32px] font-black text-xl active:scale-95 transition-all text-center">更新檔案</button>
                       </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex justify-between items-end mb-20 text-slate-900 text-left">
                   <div className="text-left"><h1 className="text-5xl font-black tracking-tighter uppercase italic text-slate-900">Engine Core</h1><div className="h-1.5 w-20 bg-amber-500 mt-4 text-left" /></div>
                   <div className="flex gap-4">
                      ...
                      <button onClick={() => { setIsAdmin(false); setIsLogged(false); setAdminPass(''); }} className="bg-white border-2 px-10 py-5 rounded-[30px] font-black flex items-center gap-3 shadow-lg active:scale-95 transition-all text-slate-900">Close</button>
                   </div>
                </div>
                ...
                <div className="space-y-10 text-left text-slate-900">
                   <div className="bg-white p-12 rounded-[60px] shadow-sm border border-slate-100 flex items-center justify-between group hover:border-indigo-200 transition-all text-left">
                      ...
                      <button onClick={() => setEditingStage({ id: 'NEW', order: game.stages.length+1, title: '新任務', speaker: '白鸞卿', storyContent: '請輸入故事...', unlockType: 'PASSWORD', unlockAnswer: '1234', hints: [''], successMessage: 'Success' })} className="bg-indigo-600 text-white p-7 rounded-[35px] shadow-2xl hover:rotate-90 transition-all active:scale-90 flex items-center justify-center text-center"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg></button>
                   </div>
                   <div className="grid gap-6 text-left">
                      {game.stages.map((s, i) => (
                        <div key={s.id} className="bg-white p-10 rounded-[60px] border-2 border-transparent hover:border-indigo-100 flex items-center gap-10 transition-all group shadow-sm text-slate-900 text-left">
                           ...
                           <div className="flex-1 text-left"><h4 className="text-2xl font-black text-slate-800 tracking-tight text-left">{s.title}</h4><div className="flex gap-3 mt-3 text-slate-400 text-left text-[10px] font-black uppercase tracking-widest text-slate-400"><span>{s.unlockType}</span><span>Key: {s.unlockAnswer}</span></div></div>
